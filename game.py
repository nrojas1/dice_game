...
class Game:
    ...
    def round(self):
        ...
        # TODO: ADD FIRST TURN AS A PLAYER ATTR AND MAKE A FUNCTION THAT CHANGES
        # THE VALUE OF THE THING. MAYBE USE 0 AND 1 INSTEAD OF TRUE AND FALSE
...
class Player:
    ...
    def throw(self, n_dice):
        """ create a Player.hand given the number of dice """
    ...
    def above_water(self, first_turn):
        """
            Test if the player has enough points to save them as well as
            checking if it is the first turn
        """
    ...
    def throw_to_score(self, hand):
        """ Takes throw and returns its values as a game score """
        if not hand:
            sys.exit()
